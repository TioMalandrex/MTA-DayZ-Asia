...
function antispam()
	if (getElementData(localPlayer,"antispam")) then
		outputChatBox(getLanguageTextClient("clientinfotext44"), 136,8,8);
		return false;
	else
...
bindKey("f6", "down", openTeamSystem)
...
function updateGPS()
    ...
    -- Check if the player belongs to a group
    ...
    -- Create blips for players in the same group
