...
addEventHandler("receiveServerData", resourceRoot,
	function(weightLimit, chatMessages, colour)
		...
		addCommandHandler("helidrop", attemptGrabStop)
		...
		bindKey("backspace", "down", "helidrop", "")
	end
)
...
function attemptGrabStart()
	-- if they are already heligrabbing
	...
	-- prevent unnecesary execution (since jump bind is commonly used in gameplay)
	local x, y, z = getElementPosition(localPlayer)
	local nearbyVehicles = getElementsWithinRange(x, y, z, 30, "vehicle")

	-- if there's no nearby vehicle(s) (hacky with table due to an issue)
	if table.empty(nearbyVehicles) == true then
		return
	end

    -- if there's nearby vehicle(s) but not heligrab candidates (hacky with table due to an issue)
    for i, vehicle in ipairs(nearbyVehicles) do
        if not table.findHeli(nearbyVehicles) then
            return
        end
		...
		if isPedInVehicle(localPlayer) then
			...
			-- now that we performed all checks, call the first function
			...
			if target.vehicle and target.distance <= grabDistance then
				...
				local collision, _, _, _, element = processLineOfSight(target.point[1], target.point[2], target.point[3], target.point[1], target.point[2],
										target.point[3] - (groundDropDistance + 0.2), true, true, false, true, false, true, false, false,target.vehicle)
...
function grabVehicle(vehicle, side, linePercent)
	...
	if hangVehicle and
			((vehicle ~= hangVehicle) or (side ~= getElementData(source, "heligrab.side")) or
				(linePercent ~= getElementData(source, "heligrab.linePercent"))) then
	...
	setElementData(source, "heligrab.dimension", getElementDimension(localPlayer))
	setElementData(source, "heligrab.interior", getElementInterior(localPlayer))
	...
	-- aaaaa
	for key, state in pairs(getBoundKeys("jump")) do
		unbindKey(key, "down", "grab")
		bindKey(key, "down", "helidrop")
	end
...
function stopPlayerGrab(vehicle, force, reason, broadcast)
	...
	if hangVehicle or force then
		...
		if source == localPlayer then
			...
			setElementData(localPlayer, "heligrab.dimension", nil)
			setElementData(localPlayer, "heligrab.interior", nil)
		else
			if vehicle then
...
function updateGrabbingEffect()
	...
	if hangVehicle then
		if getElementInterior(localPlayer) ~= getElementData(localPlayer, "heligrab.interior") then
			triggerEvent("stopPlayerGrab", localPlayer, getElementData(localPlayer, "heligrab.vehicle"), true, "interior change")
		end
		if getElementDimension(localPlayer) ~= getElementData(localPlayer, "heligrab.dimension") then
			triggerEvent("stopPlayerGrab", localPlayer, getElementData(localPlayer, "heligrab.vehicle"), true, "dimension change")
		end
		...
		local dropX =
			offsets.x * matrix[1][1] + offsets.y * matrix[2][1] + offsets.z - feetUpDistance * matrix[3][1] + matrix[4][1]
		local dropY =
			offsets.x * matrix[1][2] + offsets.y * matrix[2][2] + offsets.z - feetUpDistance * matrix[3][2] + matrix[4][2]
		local dropZ =
			offsets.x * matrix[1][3] + offsets.y * matrix[2][3] + offsets.z - feetUpDistance * matrix[3][3] + matrix[4][3]
...
addEventHandler("onClientElementStreamIn", getRootElement(), addRender)

-- This function is designed to avoid optimization bugs, because you can't toggle rendering while there may be more than 1 heli with grabbed people in the air nearby.
-- Doing so would otherwise result in those players having a crooked appearance on your screen, because their states won't be rendered.
local function removeRender()
	if isElementAttached(localPlayer) and getElementAttachedTo(localPlayer) == source then 
		return 
	end
	if getElementType(source) == "vehicle" and getVehicleType(source) == "Helicopter" then
		local x, y, z = getElementPosition(localPlayer)
		local nearbyVehicles = getElementsWithinRange(x, y, z, 325, "vehicle")

		for i, v in ipairs(nearbyVehicles) do
			if not getElementData(v, "heligrab.vehicle") then
				removeEventHandler("onClientRender", root, onRender) -- unload
			end
		end
	end
end
addEventHandler("onClientElementStreamOut", getRootElement(), removeRender)

function onRender()
		-- check attachments, animations and rotations for all nearby players
		
		local x, y, z = getElementPosition(localPlayer)
		local nearbyPlayers = getElementsWithinRange(x, y, z, 325, "player")
		...
		for _, p in ipairs(nearbyPlayers) do
			...
			if vehicle then
				...
				if not isElementAttached(p) then
					...
					local diff =
						math.abs(helicopterOffsets[getElementModel(vehicle)].front - helicopterOffsets[getElementModel(vehicle)].back)

					attachElements(p, vehicle, x, helicopterOffsets[getElementModel(vehicle)].back +
							(diff * (math.abs(getElementData(p, "heligrab.linePercent") - 1))),
						helicopterOffsets[getElementModel(vehicle)].z)
					end
...
function ForceStopPlayerGrab(vehicle, force, reason, broadcast)
	local hangVehicle = getElementData(source, "heligrab.vehicle")
...
addEvent("ForceStopPlayerGrab", true)
addEventHandler("ForceStopPlayerGrab", root, ForceStopPlayerGrab)