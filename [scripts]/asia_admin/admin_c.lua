...
function main()
	...
	if (source == giveWindowButtonGive) then
		local target = getPlayerFromName(guiGridListGetItemText(gridlistPlayers1, guiGridListGetSelectedItem(gridlistPlayers1), 1))
		local item = guiGridListGetItemText(giveWindowGridlist, guiGridListGetSelectedItem(giveWindowGridlist), 1)
		local quantity = guiGetText(giveWindowEditboxQuant)
		if target and (guiGridListGetSelectedItem(giveWindowGridlist) ~= -1) and quantity then
		triggerServerEvent("givePlayer", localPlayer, target, item, quantity)
		else
			outputChatBox("#880808[DayZ PH]#D3D3D3 You need to select a category!",0,0,0,true)
		end
	end

	if (source == vehicleWindowButtonSpawn) then
		local target = getPlayerFromName(guiGridListGetItemText(gridlistPlayers1, guiGridListGetSelectedItem(gridlistPlayers1), 1))
		local vehicle = guiGridListGetItemText(vehicleWindowGridlist, guiGridListGetSelectedItem(vehicleWindowGridlist), 1)
		if target and (guiGridListGetSelectedItem(vehicleWindowGridlist) ~= -1) then
			...
			triggerServerEvent("vehicleEvent", localPlayer,target,vehID,engine,tire,tankpart,rotor,scrap,fuel,maxslots)
			guiSetVisible(vehicleWindow,false)
			guiSetVisible(window,false)
			showCursor(false)
		else 
			outputChatBox("#880808[DayZ PH]#D3D3D3 Select a vehicle to spawn it!",0,0,0,true)
		end
	end
end)
